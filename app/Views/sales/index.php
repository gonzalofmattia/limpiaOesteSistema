...
<div class="space-y-5">
    <form method="get" action="<?= e(url('/ventas')) ?>" class="bg-white rounded-xl border border-gray-200 p-4 grid md:grid-cols-6 gap-3">
        ...
        <input type="text" name="q" value="<?= e((string) ($filters['q'] ?? '')) ?>" placeholder="Buscar Nro venta/presupuesto" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
        ...
        <table class="min-w-full text-sm">
            ...
            <?php foreach ($sales as $sale): ?>
                <tr>
                    ...
                    <td class="px-4 py-2 text-center"><span class="inline-flex px-2 py-0.5 rounded-full text-xs <?= e((string) $sale['delivery_badge']) ?>"><?= e((string) $sale['delivery_label']) ?></span></td>
                    ...
                    <td class="px-4 py-2 text-right">
                        <a href="<?= e(url('/ventas/' . (int) $sale['id'])) ?>" class="text-[#1565C0] hover:underline">Ver</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>

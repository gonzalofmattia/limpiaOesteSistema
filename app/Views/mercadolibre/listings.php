        ...
        <div class="w-full max-w-md rounded-2xl bg-white border border-lo-border shadow-xl p-5" @click.outside="if (!syncRunning) syncConfirm = false">
            ...
            <template x-if="!syncRunning && !syncSummary && !syncError">
                <div>
                    ...
                    <p class="mt-2 text-sm text-slate-600">Se corre el motor de sync bidireccional (título, precio, stock, descripción, categoría e imágenes) sobre todos los listings activos y pausados. Los campos que cambiaron a la vez en ML y en el sistema quedan como conflicto pendiente, no se pisan solos.</p>
                    <div class="mt-5 flex justify-end gap-2">
                        ...
                        <button type="button" @click="runSyncAll()" class="px-4 py-2 rounded-lg bg-lo-blue text-sm font-semibold text-white hover:bg-blue-700">Confirmar</button>
                    </div>
                </div>
            </template>
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('mlListingsPage', () => ({
        ...
        async runSyncAll() {
            ...
            const csrfInput = document.querySelector('#ml-listings-sync-all input[name="_csrf"]');
            ...
            try {
                ...
                const handle = (data) => {
                    ...
                    if (data.type === 'start') {
                        ...
                        this.syncTotal = Number(data.total || 0);
                    ...
                    if (data.type === 'error') {
                        ...
                        this.syncError = data.error || 'Error desconocido';
                ...
                if (buffer.trim()) {
                    ...
                    try { handle(JSON.parse(buffer)); } catch (e) {}
